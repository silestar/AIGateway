...
```sql
channels (id, name, type, base_url, status, weight, max_rpm, max_tpm, max_daily_requests, test_model, last_test_latency, last_tested_at, extra_config)
```

- `weight`：渠道权重，值越大越优先
- `max_rpm`：每分钟最大请求数，0 表示不限制
- `max_tpm`：每分钟最大 Token 数，0 表示不限制
- `max_daily_requests`：每日最大请求数（每个账号），0 表示不限制
...
channel_accounts (
  ...
  api_key_prefix VARCHAR(12),  -- 密钥前缀，脱敏展示
  priority INT,       -- 越大越优先
...
flowchart TD
    ...
    G -->|是| H[调用账号池 SelectAccount<br>（优先检查 Redis 中该密钥在此渠道的绑定<br>若绑定账号 active 则直接使用<br>否则按 priority DESC 选择最佳 active 账号并建立新绑定）]
    H --> I{账号选择成功?}
    I -->|是| J[使用选定账号转发请求]
    J --> K{转发结果?}
    K -->|成功| L[返回响应]
...
4.1. **账号级速率检查（RPM/TPM/每日请求）**  
    选择账号后，检查该账号是否达到渠道配置的速率限制。限制值来自渠道的 `max_rpm`、`max_tpm`、`max_daily_requests`，但计数器按每个账号独立统计（同一渠道下每个账号各有自己的配额）。读取 Redis 计数器：
    
    - `stats:account:<account_id>:rpm:<YYYY-MM-DD-HH:MM>`（该账号当前分钟请求数）
    - `stats:account:<account_id>:tpm:<YYYY-MM-DD-HH:MM>`（该账号当前分钟 Token 数）
    - `stats:account:<account_id>:daily_requests:<YYYY-MM-DD>`（该账号当日请求数）
    
    如果任一限制超限，则跳过该账号，尝试同渠道下一个账号。若同渠道所有账号均超限，触发渠道级故障转移。
...
5. **账号选择（含粘性复用）**  
    ...
    - **绑定失效时重新选择**：若绑定不存在或账号已不可用，则从该渠道当前所有 `active` 账号中按 `priority DESC`（优先级值越大越优先）选择最佳账号，并在 Redis 中建立新绑定（TTL 默认 1 小时）。
    
    - **账号每日配额检查**：选择账号后，如果渠道的 `max_daily_requests > 0`，则检查 Redis 计数器 `stats:account:<account_id>:daily_requests:<YYYY-MM-DD>`（每个账号独立计数）。若当前计数 ≥ max_daily_requests，则跳过该账号，按优先级继续尝试同渠道下一个账号。日志记录 `daily request quota exhausted for account_id=X`。若同渠道所有账号都达到每日上限，触发渠道级故障转移。
    
    - **同渠道内账号重试**：若选定账号在转发后失败（如返回 429/5xx），账号池将该账号标记为禁用、清除绑定，然后**按优先级顺序**尝试该渠道的下一个 `active` 账号。此过程重复进行，直到找到一个可用账号并成功转发。
    
    - **429 被动熔断**：如果上游返回 HTTP 429，或者响应体中包含 `quota exceeded`、`rate limit` 等关键字，则将账号状态设为 `disabled`，并设置冷却时间到当天结束（`probe_cooldown_until = 次日 00:00:00`），避免当日再次被选中。
...
- 支持跨日自动滚动（运行过午夜时自动切换新文件）。

- 支持单文件大小限制 `max_size_mb`，超限自动切换新文件。
...
| log_type | 说明 | 触发时机 |
|----------|------|----------|
| consumption | 用户请求（消费） | proxy 转发完成后 |
| probe | 账号池探测请求 | 探测调度器/健康巡检 |
| health_check | 管理员手动测试 | TestChannel / BatchTestModels / TestSingleModel |

> 所有统计聚合和实时计数器仅统计 `consumption` 类型日志，`probe` 和 `health_check` 不影响业务统计。
...
request_logs (
    ...
    log_type VARCHAR(20) DEFAULT 'consumption',
    trace_id VARCHAR(64),
...
### 3.7 插件系统（v2.0 — 声明式钩子架构）

**插件模型**：Sidecar 进程，通过 HTTP/gRPC 与主系统通信。  
**核心设计**：钩子是系统预埋的扩展点，插件通过 `manifest.json` 的 `hooks` 字段声明式注册，无需渠道级绑定。
...
| 钩子 | 类型 | 触发时机 | 调度策略 |
|------|------|---------|---------|
| `pre_request` | request | 认证完成、路由之前 | 依次调用，任一 reject 则终止请求 |
| `post_response` | response | 上游响应返回后、发送给客户端前 | 依次调用，任一 reject 则终止请求 |
| `account_select` | selection | 账号选择前（`SelectAccount` 执行之前） | 依次调用，累计 exclude_ids |
| `connection_decorator` | connection | TLS 连接建立时 | 首个返回 proxy_addr 的插件生效 |
| `on_log` | lifecycle | 请求结束后、日志写入前 | 异步调用，忽略结果 |

#### 架构特征
- **声明式注册**：插件安装时根据 `manifest.json` 的 `hooks` 字段自动注册到内存 `hookRegistry`，插件停止时自动注销
- **优先级排序**：同一钩子下的多个插件按 `priority` 升序执行
- **无需渠道绑定**：废除 `channel_plugin_settings` 表，插件全局生效
- **声明式建表**：`manifest.json` 的 `tables` 字段声明建表需求，安装时自动创建，卸载时可选删除
- **管理员控制**：通过 WebUI 启用/禁用钩子（`PUT /api/hooks/:id/enabled`），调整插件优先级

**管理界面**：插件列表页面（上传/安装/启动/停止/卸载）+ 独立钩子管理页面（`/plugins/settings`）。

**安全认证**：插件 Token 使用全局加密密钥加密存储于数据库，启动时通过环境变量注入。

### 3.8 模型管理模块

**功能**：聚合所有活跃渠道的模型映射，提供统一的模型目录管理和 `/v1/models` 兼容端点。

**核心数据结构**：`model_catalog` 表，从 `channel_models` 自动同步，按 `display_model_name` 去重。

**分类**：
- **已选模型**：`display_model_name == actual_model_name`，即未做自定义映射的原始模型
- **自定义映射模型**：`display_model_name != actual_model_name`，即管理员自定义了别名

**同步机制**（全量刷新）：
- 触发时机：渠道 SaveModels / Delete / UpdateStatus（启用/禁用）+ 服务启动时
- 新模型 → INSERT
- 已存在且 `is_mapped=true` → 不覆盖映射标记
- 模型从所有渠道移除 → `visible=false`（保留记录，不删除）
- 模型重新出现 → 自动恢复 `visible=true`

**可见性控制**：管理员可在 WebUI 模型管理页面勾选哪些模型对外返回（`visible` 字段），隐藏的模型不会出现在 `/v1/models` 中。

**API 端点**：
- `GET /v1/models`：OpenAI 兼容格式，返回 `visible=true` 且 `ref_count > 0` 的模型
- `GET /api/models/catalog`：管理端，返回完整目录
- `PUT /api/models/catalog/:id/visibility`：切换模型可见性
