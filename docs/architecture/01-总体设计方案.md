...
- **统一 OpenAI 协议**：所有入站请求和出站响应均采用 OpenAI Chat Completions 格式，适配绝大多数下游客户端。渠道适配器负责双向协议转换（OpenAI ↔ 上游格式），对消费者完全透明。
...
- **重写**：消费者体系、路由逻辑、账号池管理、统计日志、多语言、插件机制、全局加密与环境变量配置。
...
consumer_groups (id, name, description)
consumer_group_members (consumer_id, group_id, quota_rpm, quota_tpm)
```

**功能**：生成/吊销 Key，自定义名称；实时统计（Redis 计数→定时入库）；通过消费者分组控制可访问的渠道分组。
...
- **Consumer Group 与 Channel Group 的绑定**：定义哪些用户能使用哪些渠道分组。
    
    - `consumer_group_id`， `channel_group_id`
...
```mermaid
flowchart TD
    Q[请求：消费者+模型名] --> QCheck{消费者配额检查<br>Redis 计数器是否超 quota_rpm/tpm?}
    QCheck -->|超限| R429[返回 429 Too Many Requests]
    QCheck -->|通过| A[获取消费者有权访问的渠道分组列表]
    A --> C[按 Group 权重降序排序]
    C --> D[遍历每个候选 Group]
    D --> E[取出 Group 内包含该模型的所有渠道<br>按渠道权重降序排序]
    E --> F[遍历当前 Group 的渠道列表]
    F --> G{当前渠道是否存在<br>active 账号?}
    G -->|是| H[调用账号池 SelectAccount<br>（优先检查 Redis 中该消费者在此渠道的绑定<br>若绑定账号 active 则直接使用<br>否则按 priority ASC 选择最佳 active 账号并建立新绑定）]
    H --> I{账号选择成功?}
    I -->|是| J[使用选定账号转发请求]
    J --> K{转发结果?}
    K -->|成功| L[返回响应]
...
0. **消费者配额检查**  
   在路由开始前，先查询 Redis 中该消费者的当前分钟/日计数器。若已超过 `consumer_group_members` 中为该消费者分组配置的 `quota_rpm` 或 `quota_tpm`，直接返回 `429 Too Many Requests`，不再进行后续路由和账号选择。此步骤在认证完成后、分组路由之前执行，避免超配额消费者消耗上游资源。

1. **获取消费者权限分组**  
    根据 Consumer 所属的 Consumer Group，获得允许访问的 Channel Group 列表，按 Group 权重大小降序排序。
...
5. **账号选择（含粘性复用）**  
    ...
    - **优先检查粘性绑定**：查询 Redis 中是否存在 `consumer_account_affinity:{consumer_id}:{channel_id}`。若存在且指向的账号状态仍为 `active`，则**直接复用该账号**，无需重新按优先级选择。
...
consumer_daily_stats (id, date, consumer_id, model_name,  total_requests, ...)
...
- **消费者模块**：若选择存储完整 Key（而非仅哈希），则同样加密存储。
...
consumer_group_channel_groups (consumer_group_id, channel_group_id)

-- 请求日志
request_logs (id, timestamp, consumer_id, model_name, channel_id, account_id, retry_chain JSON, stream, prompt_tokens, completion_tokens, status_code, error_msg, latency_ms, request_meta JSON, response_meta JSON)
...
consumer_daily_stats (id, date, consumer_id, model_name, total_requests, success_requests, fail_requests, avg_latency_ms, total_tokens)
