...
- **缓存友好**：同一密钥尽可能复用同一账号，利用上游语义/频率缓存。
...
channel_accounts (
    ...
    api_key_prefix VARCHAR(12) NOT NULL DEFAULT '',  -- 密钥前缀，用于脱敏展示
    priority INT NOT NULL DEFAULT 0,   -- 越大越优先
    remark VARCHAR(200) NOT NULL DEFAULT '',  -- 备注信息，方便用户识别账号用途
    ...
    probe_failures INT NOT NULL DEFAULT 0,   -- 探测连续失败计数，达 max_probe_failures 上限后停止探测
...
|键名|类型|说明|
|---|---|---|
|`consumer_account_affinity:{key_id}:{channel_id}`|String|当前绑定的 account_id，TTL 默认 1 小时|
...
|`stats:account:{account_id}:rpm:{YYYY-MM-DD-HH:MM}`|Int|账号每分钟请求数（限制值来自渠道 max_rpm）|
|`stats:account:{account_id}:tpm:{YYYY-MM-DD-HH:MM}`|Int|账号每分钟 Token 数（限制值来自渠道 max_tpm）|
|`stats:account:{account_id}:daily_requests:{YYYY-MM-DD}`|Int|账号每日请求数（限制值来自渠道 max_daily_requests）|
...
flowchart TD
    ...
    D -->|否| F[DEL affinity]
    B -->|否| F
    F --> G[查询DB: channel_id=? AND status='active'<br>ORDER BY priority DESC]
    G --> H{列表为空?}
...
- **429 被动熔断**：如果上游返回 HTTP 429，直接将账号状态设为 `disabled`，并设置冷却时间到当天结束（`probe_cooldown_until = 次日 00:00:00`），避免当日再次被选中。
...
- **密钥安全复制**：前端不渲染明文密钥。管理员需复制密钥时，点击"复制"按钮 → 弹出确认框 → 调用 `POST /api/accounts/:id/reveal-key` API → 后端解密并返回明文 → 前端直接写入剪贴板（`navigator.clipboard.writeText()`）→ 明文不进入任何前端状态变量 → 提示"已复制"。该 API 记录审计日志并设置频率限制（同一密钥每分钟最多 3 次）。
