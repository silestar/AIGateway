...
### 3.2 ChannelAdapter 完整接口定义

系统采用**统一 OpenAI 协议**策略：所有入站请求和出站响应均采用 OpenAI Chat Completions 格式。适配器的核心职责是**双向协议转换**。

```go
type ChannelAdapter interface {
    // 双向协议转换
    ConvertRequest(ctx context.Context, openaiReq *OpenAIRequest) (*UpstreamRequest, error)
    ConvertResponse(ctx context.Context, upstreamResp *UpstreamResponse) (*OpenAIResponse, error)
    ...
    // 适配器标识
    Type() string  // 返回 "openai" / "openai-response" / "anthropic" / "gemini"
}
```

**适配器类型与转换规则**：

| 适配器 | Type() | 请求转换 | 响应转换 | 流式转换 |
|--------|--------|---------|---------|---------|
| `OpenAIAdapter` | `openai` | 透传（已是目标格式） | 透传 | 透传 |
| `OpenAIResponsesAdapter` | `openai-response` | OpenAI Chat → Responses API 格式 | Responses API → OpenAI Chat 格式 | SSE 格式转换 |
| `AnthropicAdapter` | `anthropic` | OpenAI Chat → Anthropic Messages 格式 | Anthropic Messages → OpenAI Chat 格式 | `content_block_delta` → `choices.delta` |
| `GeminiAdapter` | `gemini` | OpenAI Chat → Gemini `generateContent` 格式 | Gemini → OpenAI Chat 格式 | Gemini 流式 → SSE `choices.delta` |

> **`openai` 类型适配器**是最常用的，因为大量第三方服务（Azure OpenAI、DeepSeek、Moonshot、Groq、one-api 等）都兼容 OpenAI 格式。该适配器直接透传，零转换开销。
...
其他渠道（Anthropic、Gemini 等）各自实现相应解析逻辑。若某渠道不支持模型列表接口（如自定义代理），`FetchModels` 可返回 `ErrNotSupported`，后端将提示管理员手动添加。
...
{
  "models": [
    { "actual_model_name": "gpt-4o", "enabled": true, "display_name": "" },
    { "actual_model_name": "gpt-4o-mini", "enabled": true, "display_name": "my-fast-model" }
  ]
}
```
...
- 开启事务，对 `channel_models` 表进行 Upsert 操作（基于 `channel_id + actual_model_name` 唯一键）。
...
- 若管理员未设置 `display_name`（即为空），则 `display_model_name` 在表中存储为 `actual_model_name`。
...
2. **账号优先级与可用性**：从权重最高的渠道开始，检查该渠道是否有 `active` 账号，若有，则调用账号池选择最佳账号（粘性优先，否则按 `priority DESC`，值越大越优先）。若无，则跳过。
